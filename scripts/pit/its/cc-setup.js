...
(async () => {
    ...
    await takeScreenshot(page, arg, __filename, 'login-form-loading');
    await expect(page.getByLabel('Email')).toBeVisible();
    await takeScreenshot(page, arg, __filename, 'login-form-ready');
    ...
    if (await newsPass.count() == 0) {
        ...
        await takeScreenshot(page, arg, __filename, 'password-changed');

        await page.getByLabel('First Name').fill(arg.login.split('@')[0]);
        await page.getByLabel('Last Name').fill(arg.login.split('@')[1]);
        await page.getByRole('button', { name: 'Submit' }).click();
        await takeScreenshot(page, arg, __filename, 'user-configured');
        await waitForServerReady(page, arg.url, arg);
