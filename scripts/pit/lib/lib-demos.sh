...
getInstallCmdDev() {
  case $1 in
    *-gradle) echo "$GRADLE clean" ;;
    ...
    use-cases) echo "$MVN -ntp -B clean install -DskipTests -pl signals -am $PNPM";;
...
getInstallCmdPrd() {
  ...
  case $1 in
    ...
    *-quarkus) echo "$MVN -ntp -B clean package -Pproduction $E $PNPM -Dquarkus.analytics.disabled=true";;
    ...
    use-cases) echo "$H -pl signals -am $PNPM";;
...
getRunCmdDev() {
  ...
  case $1 in
    ...
    *-quarkus) echo "$MVN -ntp -B $PNPM -Dquarkus.analytics.disabled=true -Dquarkus.http.port=$2";;
    ...
    skeleton-starter-flow-cdi) W=""; [ -n "$WILDFLY_HOME" ] && W="-Djboss-as.home=$WILDFLY_HOME"; echo "$MVN -ntp -B wildfly:run $W $PNPM -Djboss.http.port=$2";;
    ...
    *-gradle) echo "$GRADLE bootRun --args='--server.port=$2'";;
    ...
    use-cases) echo "$MVN -ntp -B spring-boot:run -pl signals -Dspring-boot.run.arguments=--server.port=$2";;
...
getRunCmdPrd() {
  ...
  case $1 in
    ...
    *addon-template|addon-starter-flow) echo "$MVN -ntp -Pproduction -B -Djetty.http.port=$2 jetty:run";;
    ...
    use-cases) echo "java $_P -jar signals/target/*.jar";;
